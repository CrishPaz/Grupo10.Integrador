...
const debouncedSearch = () => {
  ...
  // Simulamos búsqueda (para que funcione ya mismo sin backend)
  // Búsqueda real al backend
  searchTimeout = setTimeout(async () => {
    try {
        const response = await $fetch<{ patients: any[] }>(`/api/patients/search?q=${searchTerm.value}`)
        
        // Mapear respuesta del API al formato del componente
        results.value = (response.patients || []).map(p => ({
          id: p.id, // ID del Paciente
          dni: p.usuarioUsuario.dni,
          nombres: p.usuarioUsuario.nombres,
          apellidos: p.usuarioUsuario.apellidos,
          empresa: p.empresa,
          ultimo_estado: 'programado'
        }))
...
const registerNewPatient = () => {
  // Navigate to admissions page with flag to open new patient modal
